...
type mp_real
  sequence
  integer (mpiknd):: mpr(0:mpwds6-1)
end type
type mp_complex
  sequence
  integer (mpiknd):: mpc(0:2*mpwds6-1)
end type
...
type mp_realm
  sequence
  integer (mpiknd):: mpr(0:mpwdsm6-1)
end type
type mp_complexm
  sequence
  integer (mpiknd):: mpc(0:2*mpwdsm6-1)
end type
...
!  Addition routines:
  ...
  function mp_adddz (da, zb)
    ...
    z1%mpc(0) = mpwds6
    z1%mpc(l2) = mpwds6
    mp_adddz%mpc(0) = mpwds6
    mp_adddz%mpc(l2) = mpwds6
    d1 = 0.d0
  ...
  function mp_addzd (za, db)
    ...
    z1%mpc(0) = mpwds6
    z1%mpc(l2) = mpwds6
    mp_addzd%mpc(0) = mpwds6
    mp_addzd%mpc(l2) = mpwds6
    d1 = 0.d0
...
!  Subtraction routines:
  ...
  function mp_subdz (da, zb)
    ...
    z1%mpc(0) = mpwds6
    z1%mpc(l2) = mpwds6
    mp_subdz%mpc(0) = mpwds6
    mp_subdz%mpc(l2) = mpwds6
    d1 = 0.d0
    call mpdmc40 (da, 0, z1%mpc, mpnw)
    call mpdmc40 (d1, 0, z1%mpc(l2:), mpnw)
  ...
  function mp_subzd (za, db)
    ...
    z1%mpc(0) = mpwds6
    z1%mpc(l2) = mpwds6
    mp_subzd%mpc(0) = mpwds6
    mp_subzd%mpc(l2) = mpwds6
    d1 = 0.d0
    call mpdmc40 (db, 0, z1%mpc, mpnw)
    call mpdmc40 (d1, 0, z1%mpc(l2:), mpnw)
...
!  Division routines:
  ...
  function mp_divdz (da, zb)
    ...
    z1%mpc(0) = mpwds6
    z1%mpc(l2) = mpwds6
    mp_divdz%mpc(0) = mpwds6
    mp_divdz%mpc(l2) = mpwds6
    d1 = 0.d0
    call mpdmc40 (da, 0, z1%mpc, mpnw)
    call mpdmc40 (d1, 0, z1%mpc(l2:), mpnw)
  ...
  function mp_divzd (za, db)
    ...
    z1%mpc(0) = mpwds6
    z1%mpc(l2) = mpwds6
    mp_divzd%mpc(0) = mpwds6
    mp_divzd%mpc(l2) = mpwds6
    d1 = 0.d0
    call mpdmc40 (db, 0, z1%mpc, mpnw)
    call mpdmc40 (d1, 0, z1%mpc(l2:), mpnw)
...
!    mpnw = mp_setwp (iprec)
!>>
    mp_mtor%mpr(0) = mpwds6
    call mpeq (ra%mpr, mp_mtor%mpr, mpnw)
    return
  end function
...
!>>
  ...
  function mp_polylog_neg (nn, arr, ra)
    ...
    integer mpnw, n1
    mpnw = min (int (ra%mpr(1)), mpwds)
    mp_polylog_neg%mpr(0) = mpwds6
    n1 = mpwds
    call mppolylogneg (n1, nn, arr(1)%mpr, ra%mpr, mp_polylog_neg%mpr, mpnw)
    return
  end function
...
!>>
    l2 = mpwds6
    mp_zmtoz%mpc(0) = mpwds6
    mp_zmtoz%mpc(l2) = mpwds6
    call mpceq (za%mpc, mp_zmtoz%mpc, mpnw)
    return
  end function
